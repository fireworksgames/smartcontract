...
import TokenArtifacts from "../../artifacts/contracts/FireToken.sol/FireToken.json";
import { FireToken } from "../../typechain";
...
describe("ERC20", () => {
  ...
  const TEN_BILLION_MILLION_TOKENS: BigNumber = getBigNumber(10_000_000_000);
  const ZERO_ADDRESS = "0x0000000000000000000000000000000000000000";
  const one_hundred = getBigNumber(100);

  async function makeSUT() {
    return (await deployContract(deployer, TokenArtifacts, [])) as FireToken;
  }
  ...
  it("should initialize as expected", async function () {
    ...
    await _token.mint(deployer.address, TEN_BILLION_MILLION_TOKENS);
    expect(await _token.name()).to.be.equal("FireToken");
    expect(await _token.symbol()).to.be.equal("FIRE");
    expect(await _token.decimals()).to.be.equal(18);
    expect(await _token.totalSupply()).to.be.equal(TEN_BILLION_MILLION_TOKENS);
  });

  it("should distribute tokens correctly", async function () {
    expect(await token.balanceOf(deployer.address)).to.be.equal(TEN_BILLION_MILLION_TOKENS);
  });
  ...
  describe("balanceOf", () => {
    it("should correctly return user balance", async function () {
      ...
      expect(await token.balanceOf(deployer.address)).to.be.equal(TEN_BILLION_MILLION_TOKENS.sub(1007));
    });
  });
